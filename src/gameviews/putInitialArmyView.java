...
public class putInitialArmyView extends JDialog{
	 ...
	 */
	public putInitialArmyView(RiskGame myGame){
		...
		for (int i=0;i<localCountries[curPlayer].length;i++) { 
			CountryModel loopCountry = gameMap.findCountry(localCountries[curPlayer][i].name);
	...
	private void byComputer() { 
		SwingWorker<Void, Integer> worker = new SwingWorker<Void, Integer>() {
			...
			protected Void doInBackground() {
				do {
					int children = localCountries[curPlayer].length;
					int randomCountryIndex = (int)(Math.random()*children);
					totalInitialArmies--;
					leftArmies[curPlayer]--;
					localCountries[curPlayer][randomCountryIndex].Number++;
					try {
						Thread.sleep(50);
