...
public class PhaseViewRemote extends JFrame{
	...
	private ObserverLabel gameStageLabel4;
	...
	private void reloadPlayers(int index){
		...
		labelPlayers[index].setText("Player"+(index+1)+" ("+
				myGame.getPlayers()[index].getTotalArmies()+" armies, "+
	 ...
	 */	
	public void update(Observable obs, int x) throws RemoteException{
		Dimension size;
		int type = (Integer) x;
		if (type == 0){//update for top level phase information. startup and game started
			...
			switch (stage){
				...
				case 21://ready to assign countries randomly
					...
					break;
				case 30://ready to palce initial armies
					reloadContinents();
					reloadPlayers();
			...
			playerPhaseLabel.setText(((PlayerModel)obs).getPhaseString());
			...
			gameStageLabel4.setText((((PlayerModel)obs).getPutArmyStr()));
			gameStageLabel4.setVisible(true);
			gameStageLabel4.setForeground(Color.RED);
			size = gameStageLabel4.getPreferredSize();
 ...
 */
class ObserverLabel extends JLabel {
	private static final long serialVersionUID = 1L;
	/**
	...
	public void update(Observable arg0, Object arg1) {
		...
		this.setBounds(getBounds().x,getBounds().y,size.width,size.height);
	}
}